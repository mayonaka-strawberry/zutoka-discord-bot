...
async def effect_04_034(
    engine: EffectEngine, game_state: GameState, player_index: int, card_instance: CardInstance,
) -> None:
    """Attack +30 if the opponent's character's attack is 0."""
    log.debug('[%s] %s: entering effect_04_034', card_instance.card.effect, engine.player_label(player_index))
    ...
    if opponent_attack == 0:
        ...
        await engine._send_dm(player_index, content="**Effect (04-034):** Opponent's character attack is 0. Attack +30!")
        await engine._send_dm(opponent_index, content='**Effect (04-034):** Your character attack is 0. Opponent gains Attack +30.')
    else:
        await engine._send_dm(player_index, content=f"**Effect (04-034):** Opponent's character attack is {opponent_attack}. No effect.")
