...
async def effect_02_100(engine: EffectEngine, game_state: GameState, player_index: int, card_instance: CardInstance) -> None:
    """+10 to your attack power and the opponent's character attribute changes to flame while on the battlefield."""
    ...
    if opponent.battle_zone is not None:
        ...
        opponent.battle_zone.attribute_override = Attribute.FLAME
        log.debug('[%s] %s: opponent character attribute changed from %s to Flame', card_instance.card.effect, engine.player_label(player_index), old_attr)
    else:
