...
async def effect_03_022(engine: EffectEngine, game_state: GameState, player_index: int, card_instance: CardInstance) -> None:
    ...
    if REQUIRED_ATTRIBUTES.issubset(abyss_attributes):
        ...
        log.debug('[%s] %s: all 4 required attributes present, attack bonus +40 (%d -> %d)', card_instance.card.effect, engine.player_label(player_index), old_bonus, engine.turn_state.attack_bonus[player_index])
    else:
