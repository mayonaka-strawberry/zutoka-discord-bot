...
async def effect_01_053(engine: EffectEngine, game_state: GameState, player_index: int, card_instance: CardInstance) -> None:
    ...
    if game_state.day_night == Chronos.NIGHT:
        ...
        log.debug('[%s] %s: it is not NIGHT (day_night=%s), no bonus', card_instance.card.effect, engine.player_label(player_index), game_state.day_night)
