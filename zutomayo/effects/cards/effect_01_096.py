...
async def effect_01_096(
    engine: EffectEngine, game_state: GameState, player_index: int, card_instance: CardInstance,
) -> None:
    """Attack +10 when day changes to night."""
    ...
    if dn_at_start == Chronos.DAY and game_state.day_night == Chronos.NIGHT:
        ...
        log.debug('[%s] %s: day->night transition detected, +10 attack bonus (now %d)', card_instance.card.effect, engine.player_label(player_index), engine.turn_state.attack_bonus[player_index])
    else:
