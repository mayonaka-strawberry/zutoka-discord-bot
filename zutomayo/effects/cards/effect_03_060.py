...
async def effect_03_060(engine: EffectEngine, game_state: GameState, player_index: int, card_instance: CardInstance) -> None:
    ...
    if player.abyss and all(card_instance.card.attribute == Attribute.WIND for card_instance in player.abyss):
        ...
        log.debug('[%s] %s: all abyss cards are wind, attack bonus +70 (%d -> %d)', card_instance.card.effect, engine.player_label(player_index), old_bonus, engine.turn_state.attack_bonus[player_index])
    else:
