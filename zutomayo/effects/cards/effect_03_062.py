...
async def effect_03_062(engine: EffectEngine, game_state: GameState, player_index: int, card_instance: CardInstance) -> None:
    """Attack +50 if there are cards of 3 different attributes in the Abyss."""
    ...
    if len(abyss_attributes) >= 3:
        ...
        log.debug('[%s] %s: 3+ attributes present, attack bonus +50 (%d -> %d)', card_instance.card.effect, engine.player_label(player_index), old_bonus, engine.turn_state.attack_bonus[player_index])
    else:
