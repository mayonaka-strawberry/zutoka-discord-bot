...
async def effect_04_063(
    ...
    log.debug('[%s] %s: entering effect_04_063', card_instance.card.effect, engine.player_label(player_index))
    player = game_state.players[player_index]
    opponent_index = 1 - player_index

    flame_cards = [
        hand_card for hand_card in player.hand
        if hand_card.card.attribute == Attribute.FLAME
    ]

    if not flame_cards:
        await engine._send_dm(player_index, content='**Effect (04-063):** No flame cards in hand. No effect.')
    ...
    selection_count = await engine._prompt_number_selection(
        player_index,
        min_value=0,
        max_value=len(flame_cards),
        prompt_text=f'**Effect (04-063):** You have {len(flame_cards)} flame card(s) in hand. How many do you want to place into the Abyss?',
    ...
    if not selection_count:
        await engine._send_dm(player_index, content='**Effect (04-063):** No effect.')
        log.debug('[%s] %s: early return, skipping effect', card_instance.card.effect, engine.player_label(player_index))
        return

    # Select cards individually
    remaining_candidates = list(flame_cards)
    ...
    for selection_number in range(1, selection_count + 1):
        selected_card = await engine._prompt_card_selection(
            ...
            prompt_text=f'**Effect (04-063):** Choose flame card #{selection_number} of {selection_count} to place into the Abyss.',
            placeholder='Select a flame card...',
        )
    ...
    if not selected_cards:
        await engine._send_dm(player_index, content='**Effect (04-063):** No cards selected. No effect.')
        log.debug('[%s] %s: early return, skipping effect', card_instance.card.effect, engine.player_label(player_index))
        return
    ...
    await engine._send_dm(player_index, content=f'**Effect (04-063):** Placed {len(selected_cards)} card(s) into the Abyss: {placed_names}.')
    await engine._send_dm(opponent_index, content=f'**Effect (04-063):** Opponent placed {len(selected_cards)} flame card(s) into their Abyss.')
