...
async def effect_01_031(engine: EffectEngine, game_state: GameState, player_index: int, card_instance: CardInstance) -> None:
    """Attack +50 if the attribute of the opponent's character card is flame."""
    ...
    if opponent.battle_zone is not None and opponent.battle_zone.effective_attribute == Attribute.FLAME:
        ...
        log.debug('[%s] %s: opponent attribute is FLAME, +50 attack bonus (now %d)', card_instance.card.effect, engine.player_label(player_index), engine.turn_state.attack_bonus[player_index])
    else:
