...
async def effect_03_040(engine: EffectEngine, game_state: GameState, player_index: int, card_instance: CardInstance) -> None:
    """Attack +20 if all the cards in the Power Charger are of the flame attribute."""
    ...
    if player.power_charger and all(card_instance.card.attribute == Attribute.FLAME for card_instance in player.power_charger):
        ...
        log.debug('[%s] %s: all power charger cards are flame, attack bonus +20 (%d -> %d)', card_instance.card.effect, engine.player_label(player_index), old_bonus, engine.turn_state.attack_bonus[player_index])
    else:
