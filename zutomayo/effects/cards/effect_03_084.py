...
async def effect_03_084(engine: EffectEngine, game_state: GameState, player_index: int, card_instance: CardInstance) -> None:
    ...
    if player.abyss and all(card_instance.card.attribute == Attribute.FLAME for card_instance in player.abyss):
        ...
        log.debug('[%s] %s: all abyss cards are flame, attack bonus +70 (%d -> %d)', card_instance.card.effect, engine.player_label(player_index), old_bonus, engine.turn_state.attack_bonus[player_index])
    else:
