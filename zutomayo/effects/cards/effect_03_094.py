...
async def effect_03_094(engine: EffectEngine, game_state: GameState, player_index: int, card_instance: CardInstance) -> None:
    """Specify the name of a card. Choose and reveal 1 card from the opponent's hand. If it matches, Attack +40."""
    ...
    if not opponent.hand:
        ...
        await engine._send_dm(player_index, content='**Effect (03-094):** No effect.')
        return
    ...
    specified_id = await engine._prompt_text_input(
        player_index,
        prompt_text='**Effect (03-094):** Enter a card ID (XX-XXX) to guess.',
    ...
    selected_index = await engine._prompt_number_selection(
        ...
        min_value=1,
        max_value=hand_size,
        prompt_text=f"**Effect (03-094):** You specified: **{specified_id}**\nChoose a card from the opponent's hand:",
    ...
    if specified_id.strip() == revealed_id:
        ...
        log.debug('[%s] %s: match! specified=%s == revealed=%s, attack bonus +40 (%d -> %d)', card_instance.card.effect, engine.player_label(player_index), specified_id.strip(), revealed_id, old_bonus, engine.turn_state.attack_bonus[player_index])
        await engine._send_dm(player_index, content='**Effect (03-094):** Match! Attack +40!')
    else:
        ...
        await engine._send_dm(player_index, content='**Effect (03-094):** No match. No bonus.')
