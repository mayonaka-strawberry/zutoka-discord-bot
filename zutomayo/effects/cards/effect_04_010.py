...
async def effect_04_010(
    engine: EffectEngine, game_state: GameState, player_index: int, card_instance: CardInstance,
) -> None:
    """Reveal any number of (TAIDADA) characters from your hand, Attack +20 for each."""
    log.debug('[%s] %s: entering effect_04_010', card_instance.card.effect, engine.player_label(player_index))
    ...
    if not taidada_characters:
        await engine._send_dm(player_index, content='**Effect (04-010):** No TAIDADA characters in hand. No effect.')
