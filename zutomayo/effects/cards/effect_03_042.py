...
async def effect_03_042(engine: EffectEngine, game_state: GameState, player_index: int, card_instance: CardInstance) -> None:
    """Attack +50 if the opponent's HP is 100."""
    ...
    if opponent.hp == 100:
        ...
        log.debug('[%s] %s: opponent HP is 100, attack bonus +50 (%d -> %d)', card_instance.card.effect, engine.player_label(player_index), old_bonus, engine.turn_state.attack_bonus[player_index])
    else:
