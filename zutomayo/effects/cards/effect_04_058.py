...
async def effect_04_058(
    ...
    log.debug('[%s] %s: entering effect_04_058', card_instance.card.effect, engine.player_label(player_index))
    player = game_state.players[player_index]
    opponent_index = 1 - player_index

    wind_cards = [
        hand_card for hand_card in player.hand
        if hand_card.card.attribute == Attribute.WIND
    ]

    if not wind_cards:
        await engine._send_dm(player_index, content='**Effect (04-058):** No wind cards in hand. No effect.')
    ...
    selected_card = await engine._prompt_card_selection(
        player_index,
        wind_cards,
        prompt_text='**Effect (04-058):** Choose a wind card from your hand to place into the Abyss.',
        placeholder='Select a wind card...',
    )
    ...
    if selected_card is None:
        await engine._send_dm(player_index, content='**Effect (04-058):** No card selected. No effect.')
        log.debug('[%s] %s: early return, skipping effect', card_instance.card.effect, engine.player_label(player_index))
        return
