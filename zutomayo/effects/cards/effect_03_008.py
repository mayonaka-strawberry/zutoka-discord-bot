...
async def effect_03_008(engine: EffectEngine, game_state: GameState, player_index: int, card_instance: CardInstance) -> None:
    """Attack +100 if there are cards of 4 different attributes in the Abyss."""
    ...
    if len(abyss_attributes) >= 4:
        ...
        log.debug('[%s] %s: all 4 attributes present, attack bonus +100 (%d -> %d)', card_instance.card.effect, engine.player_label(player_index), old_bonus, engine.turn_state.attack_bonus[player_index])
    else:
