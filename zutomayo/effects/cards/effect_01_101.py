...
async def effect_01_101(
    ...
    if opponent.battle_zone is not None and opponent.battle_zone.card.power_cost <= 1:
        ...
        log.debug('[%s] %s: opponent power_cost=%d <= 1, +20 attack bonus (now %d)', card_instance.card.effect, engine.player_label(player_index), opponent.battle_zone.card.power_cost, engine.turn_state.attack_bonus[player_index])
    else:
