...
async def effect_02_031(engine: EffectEngine, game_state: GameState, player_index: int, card_instance: CardInstance) -> None:
    ...
    if len(player.hand) < 2:
        ...
        await engine._send_dm(
            player_index,
            content='**Effect (02-031):** Not enough cards in hand. Effect fizzles.',
        )
        return
    ...
    first = await engine._prompt_card_selection(
        player_index,
        list(player.hand),
        prompt_text='**Effect (02-031):** Choose the first card from your hand to place at the bottom of your deck.',
    ...
    if first is None:
        ...
        await engine._send_dm(player_index, content='**Effect (02-031):** No effect.')
        return
    ...
    if not remaining:
        ...
        await engine._send_dm(player_index, content='**Effect (02-031):** No second card available. No effect.')
        return

    second = await engine._prompt_card_selection(
        player_index,
        remaining,
        prompt_text='**Effect (02-031):** Choose the second card from your hand to place at the bottom of your deck.',
    ...
    for card in (first, second):
        ...
        engine.return_to_deck_bottom(card, player)
