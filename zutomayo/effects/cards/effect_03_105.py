...
async def effect_03_105(engine: EffectEngine, game_state: GameState, player_index: int, card_instance: CardInstance) -> None:
    ...
    if not opponent.hand:
        ...
        await engine._send_dm(player_index, content='**Effect (03-105):** No effect.')
        return
    ...
    specified_id = await engine._prompt_text_input(
        player_index,
        prompt_text='**Effect (03-105):** Enter a card ID (XX-XXX) to guess.',
    ...
    selected_index = await engine._prompt_number_selection(
        ...
        min_value=1,
        max_value=hand_size,
        prompt_text=f"**Effect (03-105):** You specified: **{specified_id}**\nChoose a card from the opponent's hand:",
    ...
    await engine._send_dm(player_index, content='**Effect (03-105):** Revealed card:', embed=embed, file=reveal_img)

    reveal_img = create_deck_grid_image([revealed], columns=1)
    await engine._send_dm(1 - player_index, content=f'**Effect (03-105):** Your card was revealed: {revealed_name}.', file=reveal_img)

    reveal_img = create_deck_grid_image([revealed], columns=1)
    await engine._send_to_channel(content=f"**Effect (03-105):** A card was revealed from opponent's hand: {revealed_name}.", file=reveal_img)
    ...
    if specified_id.strip() == revealed_id:
        ...
        await engine._send_dm(player_index, content='**Effect (03-105):** Match! Attack +100!')
    else:
        ...
        await engine._send_dm(player_index, content='**Effect (03-105):** No match. No bonus.')
