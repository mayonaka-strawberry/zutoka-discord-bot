...
async def effect_04_008(
    engine: EffectEngine, game_state: GameState, player_index: int, card_instance: CardInstance,
) -> None:
    """Reveal your hand. +80 Attack if it contains 4 or more attributes."""
    log.debug('[%s] %s: entering effect_04_008', card_instance.card.effect, engine.player_label(player_index))
    player = game_state.players[player_index]
    opponent_index = 1 - player_index

    if not player.hand:
        await engine._send_dm(player_index, content='**Effect (04-008):** Hand is empty. No effect.')
