...
async def effect_04_104(
    ...
    log.debug('[%s] %s: entering effect_04_104', card_instance.card.effect, engine.player_label(player_index))
    ...
    if study_me_count > 0:
        attack_bonus = 20 * study_me_count
        ...
        await engine._send_dm(
            player_index,
            content=f'**Effect (04-104):** {study_me_count} STUDY ME card(s) in Power Charger. Attack +{attack_bonus}!',
        )
        await engine._send_dm(
            opponent_index,
            content=f'**Effect (04-104):** Opponent has {study_me_count} STUDY ME card(s) in Power Charger. Attack +{attack_bonus}.',
        )
    else:
        await engine._send_dm(player_index, content='**Effect (04-104):** No STUDY ME cards in Power Charger. No effect.')
