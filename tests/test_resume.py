...
def _play_original(tmp_path: Path) -> tuple[dict, int | None, Path]:
    """Play the uninterrupted match; returns (final digest, winner, game directory)."""
    _install_patches(tmp_path)
    deck_0, deck_1 = _decks()

    session = GameSession(game_id='resume-original', channel_id=7, creator_id=PLAYER_ZERO_ID)
    session.add_player(PLAYER_ONE_ID)
    ...
    winner = asyncio.run(flow.run_single_match(session, deck_0, deck_1))

    game_directory = session.persistence.game_directory
    assert game_directory.exists(), 'persistence must be created for a 2-player match'
    assert (game_directory / 'decisions.jsonl').exists()
    return _final_digest(session), winner, game_directory


def _resume_from(game_directory: Path, tmp_path: Path, truncate_to: int | None) -> tuple[dict, int | None, TranscriptRecorder, GameSession]:
    """Copy the persisted game (optionally truncating the log) and resume it."""
    resumed_directory = tmp_path / f'resumed_{truncate_to}'
...
def test_resume_reproduces_final_state_at_many_truncation_points(tmp_path):
    ...
    for truncate_to in truncation_points:
        resumed_digest, resumed_winner, recorder, _ = _resume_from(game_directory, tmp_path, truncate_to)
        assert resumed_winner == original_winner, f'winner differs at truncation {truncate_to}'
        assert resumed_digest == original_digest, f'final state differs at truncation {truncate_to}'
        if truncate_to is None:
            assert not recorder.went_live, 'a fully logged game must finish inside replay'
        else:
            assert recorder.went_live, f'truncated log {truncate_to} must go live'
